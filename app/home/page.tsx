...
import { Download, FileText, BarChart2, Layers, Trash2 } from 'lucide-react';
...
function getCoverStyle(config: ExportRecord['config']): { bg: string; accent: string } {
  const primary = config?.coverDesign?.colors?.primary || config?.theme?.colors?.[0] || '#0f172a';
  const secondary = config?.coverDesign?.colors?.secondary || config?.theme?.colors?.[1] || '#1e3a5f';
  const accent = config?.coverDesign?.colors?.accent || config?.theme?.brandColor || config?.theme?.colors?.[2] || '#3b82f6';
  return {
...
function CoverThumbnail({ bg, accent, brand, period }: {
  bg: string; accent: string; brand: string; period: string;
}) {
  return (
    <div
...
export default function HomePage() {
  ...
  const historyPeriodOptions = Array.from(
    new Set(history.map((r) => monthYearToLabel(r.period))),
  ).sort((a, b) => {
    const toDate = (label: string) => {
      const [m, y] = label.split(' ');
      return parseInt(y) * 100 + MONTHS.indexOf(m);
    };
  ...
  const filtered = history.filter((r) => {
    ...
    const matchSearch = !q || r.name.toLowerCase().includes(q) || r.brand_name.toLowerCase().includes(q) || periodLabel.toLowerCase().includes(q);
  ...
  return (
    <div className="h-screen bg-slate-50 overflow-hidden">
      ...
      {/* ── Main area ── */}
      <div className="h-full flex flex-col pl-14 overflow-hidden">
        ...
        <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 shrink-0 shadow-sm">
          <div>
            ...
            <h2 className="text-base font-bold text-slate-900">
              {getGreeting()}, <span className="text-teal-700">{user?.name ?? '...'}</span>
            </h2>
          </div>
            ...
            <div className="w-64 shrink-0 flex flex-col gap-4">
              ...
              <div className="grid grid-cols-1 gap-3">
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-[11px] text-slate-500">Total Exports</p>
                  </div>
                </div>
                <div className="bg-white rounded-xl border border-slate-200 shadow-sm p-4 flex items-center gap-3">
                  ...
                  <div>
                    <p className="text-2xl font-bold text-slate-900">{activeBrands.length}</p>
                    <p className="text-[11px] text-slate-500">Active Brands</p>
                  </div>
                </div>
              </div>
              ...
              <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
                <div className="px-4 py-3 border-b border-slate-100 flex items-center gap-2">
                  <h3 className="text-xs font-semibold text-slate-700 flex-1">Active Brands</h3>
                  <select
                    value={activePeriod}
                    onChange={(e) => setActivePeriod(e.target.value)}
                    className="text-[10px] border border-slate-200 rounded px-1.5 py-1 bg-white text-slate-600 focus:outline-none focus:ring-1 focus:ring-teal-500 cursor-pointer"
                  >
                    {last3.map((m) => (
                ...
                <div className="overflow-y-auto max-h-80">
                  {brandsLoading ? (
                    <p className="px-4 py-3 text-[11px] text-slate-400 italic">Loading...</p>
                  ) : activeBrands.length === 0 ? (
                    <p className="px-4 py-3 text-[11px] text-slate-400 italic">No active brands</p>
                  ) : (
                    activeBrands.map((brand) => (
                      <div key={brand.id} className="px-4 py-2.5 hover:bg-slate-50 transition-colors border-b border-slate-50 last:border-0">
                        ...
                        <div className="flex gap-1 pl-3.5">
                          {CHANNELS.map(({ key, label }) => {
                            ...
                            return (
                              <div key={label} title={`${label}: @${username}`}
                                className="flex items-center gap-0.5 px-1.5 py-0.5 rounded text-[9px] font-bold border bg-teal-50 text-teal-700 border-teal-200">
                ...
                <div className="px-4 py-2 bg-slate-50 border-t border-slate-100">
                  <span className="text-[10px] text-slate-400">{activeBrands.length} brand active</span>
                </div>
              </div>
            </div>
              ...
              <div className="flex items-center justify-between mb-4">
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="border border-slate-200 rounded-lg px-2 py-1.5 text-sm bg-white text-slate-600 focus:outline-none focus:ring-2 focus:ring-teal-600/30 focus:border-teal-600 transition cursor-pointer"
                  >
                    <option value="all">All Periods</option>
                    {historyPeriodOptions.map((m) => (
                      <option key={m} value={m}>{m}</option>
                    ))}
                  </select>
                  <input
                    ...
                    className="border border-slate-200 rounded-lg px-3 py-1.5 text-sm bg-white text-slate-700 placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-teal-600/30 focus:border-teal-600 w-36 transition"
                  />
                </div>
              </div>
                    ...
                    return (
                      <div key={record.id}
                        ...
                        {record.cover_image_url ? (
                          <img src={record.cover_image_url} alt={record.brand_name} className="w-full aspect-video rounded-t-lg object-cover" />
                        ) : (
                          <CoverThumbnail bg={bg} accent={accent} brand={record.brand_name} period={periodLabel} />
                        )}
                        <div className="p-3 flex flex-col gap-1.5 flex-1">
                          <p className="text-xs font-semibold text-slate-800 leading-tight line-clamp-2">{record.name}</p>
                        ...
                        <div className="px-3 pb-3 flex gap-1.5">
                          <button
                            ...
                            className="flex-1 flex items-center justify-center gap-1.5 text-[11px] font-semibold text-white bg-teal-700 hover:bg-teal-800 disabled:bg-slate-200 disabled:text-slate-400 rounded-md py-1.5 transition-colors cursor-pointer disabled:cursor-not-allowed"
                          >
                            <Download size={11} />
