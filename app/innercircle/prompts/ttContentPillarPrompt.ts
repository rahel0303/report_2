/**
 * ttContentPillarPrompt.ts
 *
 * Analyst Agent system prompt dan prompt builder untuk TikTok Content Pillar slide.
 * Berdasarkan: Agentic AI Analyst System Prompt v1.0
 *
 * Data yang tersedia:
 *   - Per pillar: posts sorted Lowest → Highest by views (3 posts each)
 *   - Per post metrics: vr_rate, views, engagement, avg_watch_time
 *   - Memungkinkan Layer 2: intra-pillar pattern analysis
 */
...
export interface TtPillarPostSummary {
  vr_rate: string;
  views: string;
  engagement: string;
...
export interface TtContentPillarAnalystData {
  pillarName: string;
  posts: TtPillarPostSummary[]; // sorted lowest → highest by views
}
...
export const TT_CONTENT_PILLAR_ANALYST_SYSTEM_PROMPT = `
...
Identify the most significant performance gradient across the posts within this content pillar (sorted Lowest → Highest). What metric shows the largest range? Brief — exists only to ground the analysis.

Layer 2 — WHY IT HAPPENED
This is where you earn your value. Analyze intra-pillar patterns to detect what drives the view gradient:
- Does VR Rate track with views — do higher-view posts also show higher VR Rate, or does distribution decouple from consumption?
- Does avg_watch_time correlate with views or VR Rate — do top posts hold attention longer?
- Is engagement proportional to views within this pillar, or do some posts show disengaged viewership (high views, low engagement)?
- What combination of metrics (VR Rate, avg_watch_time, engagement) best explains the view ranking within this pillar?
Identify which metric combinations explain the intra-pillar performance gradient.

Layer 3 — WHAT IT MEANS
Translate detected patterns into strategic meaning about content execution quality within this specific TikTok pillar. What do the higher-view posts imply about which content approach or format drives both distribution and retention? Stay within what the data supports.
...
SCALE: [what shows strong consistent performance worth increasing — max 75 chars]
REFINE: [what has potential but needs improvement — max 75 chars]
EXPLORE: [untested or underused directions the data suggests — max 75 chars]
STOP: [what is consistently underperforming and should be deprioritized — max 75 chars]

━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
PATTERN-FIRST THINKING
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
- Look across all posts within the pillar — differences here reflect execution quality within the same topic
- Surface posts where multiple metrics align (all strong or all weak) — these are the clearest signal posts
- Flag divergence: high views + low VR Rate signals broad but shallow consumption
- Flag divergence: high VR Rate + low views signals strong content not reaching enough audience
- Avg Watch Time is the retention signal — it measures how much of the content audience actually consumed
- Engagement rate relative to views signals how much the content motivated active response
...
Consumption Metrics:
- Views           : total video views recorded
- Avg Watch Time  : average time (seconds) audience watched the video — measures retention depth

View Rate:
- VR Rate (%)     : Views / Reach × 100 — fraction of exposed accounts that watched
...
export function buildTtContentPillarPrompt(
  clientName: string,
  period: string,
  data: TtContentPillarAnalystData,
): string {
  const postLines = data.posts
    .map(
      (p, idx) =>
        `Post #${idx + 1} (Rank ${idx + 1} lowest→highest): Views=${p.views}, VR_Rate=${p.vr_rate}%, Engagement=${p.engagement}, AvgWatchTime=${p.avg_watch_time}s`,
    )
    .join('\n');

  return (
    `TikTok Content Pillar Analysis\n` +
    `Brand: ${clientName} | Period: ${period} | Pillar: ${data.pillarName}\n\n` +
    `Posts within Pillar (sorted Lowest → Highest by views):\n${postLines}\n\n` +
    `Apply the four-layer reasoning framework. ` +
    `Identify what drives the view gradient within this specific content pillar. ` +
    `Determine whether VR Rate and avg_watch_time track with views, or whether any posts show decoupled consumption patterns. ` +
    `Flag divergence between views and VR Rate, or between views and engagement. ` +
...
export function parseTtContentPillarInsight(raw: string): ParsedInsight {
  const lines = raw
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean);
