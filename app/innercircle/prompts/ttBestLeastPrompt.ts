/**
 * ttBestLeastPrompt.ts
 *
 * Analyst Agent system prompt dan prompt builder untuk TikTok Best vs Least slide.
 * Berdasarkan: Agentic AI Analyst System Prompt v1.0
 *
 * Data yang tersedia:
 *   - Highest 3 posts vs Lowest 3 posts by views
 *   - Per post metrics: views, engagement, vr_rate (View Rate %)
...
export interface TtPostMetricSummary {
  rank: number;
  views: string;
  engagement: string;
  vr_rate: string;
...
export interface TtBestLeastData {
  highest: TtPostMetricSummary[];
  lowest: TtPostMetricSummary[];
}
...
export const TT_BEST_LEAST_ANALYST_SYSTEM_PROMPT = `
...
Identify the most significant performance gap between the Highest 3 and Lowest 3 posts. What is the dominant difference in views or VR Rate between the two groups? Brief — exists only to ground the analysis.

Layer 2 — WHY IT HAPPENED
This is where you earn your value. Cross-compare Highest vs Lowest to detect patterns:
- Does the Highest group show higher views AND higher VR Rate, or only higher views at similar VR Rate?
- Is engagement proportional to views in both groups, or does one group show decoupled engagement (high views, low engagement)?
- Is there a pattern in VR Rate between groups — does consumption depth differentiate the groups more than raw views?
...
Translate detected patterns into strategic meaning about content-audience fit for this TikTok account. What does the gap between Highest and Lowest imply about which content approaches drive view volume and audience retention? Stay within what the data supports.
...
- Flag divergence: high views + low VR Rate signals broad reach with shallow consumption
- Flag divergence: low views + high VR Rate signals deep engagement with limited distribution
- Views gap magnitude between groups is the primary scale signal — use it to assess performance divide
- VR Rate is the consumption depth signal — weight it to determine whether views translate to watch time
...
Consumption Metrics:
- Views        : total number of video views recorded

Engagement Metrics:
- Engagement   : total audience interactions (likes + comments + shares)

View Rate:
- VR Rate (%)  : Views / Reach × 100 — measures what fraction of exposed accounts actually watched
...
export function buildTtBestLeastPrompt(
  clientName: string,
  period: string,
  data: TtBestLeastData,
): string {
  const highestLines = data.highest
    .map(
      (p) =>
        `Highest #${p.rank}: Views=${p.views}, Engagement=${p.engagement}, VR_Rate=${p.vr_rate}%`,
    )
    .join('\n');
  ...
  const lowestLines = data.lowest
    .map(
      (p) =>
        `Lowest #${p.rank}: Views=${p.views}, Engagement=${p.engagement}, VR_Rate=${p.vr_rate}%`,
    )
    .join('\n');

  return (
    `TikTok Best vs Least Posts Analysis\n` +
    `Brand: ${clientName} | Period: ${period}\n\n` +
    `Highest 3 Posts by Views:\n${highestLines}\n\n` +
    `Lowest 3 Posts by Views:\n${lowestLines}\n\n` +
    ...
    `Determine whether the Highest group achieves more views through higher VR Rate, higher engagement, or both, and flag any divergence between views and VR Rate within either group. ` +
...
export function parseTtBestLeastInsight(raw: string): ParsedInsight {
  const lines = raw
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean);
