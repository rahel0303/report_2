 ...
 * twGrowthPrompt.ts
 *
 * Analyst Agent system prompt dan prompt builder untuk Twitter/X Growth slide.
 * Berdasarkan: Agentic AI Analyst System Prompt v1.0
 *
 * Data yang tersedia:
 *   - Monthly time series: post_count, followers_growth, impressions, engagement
...
export interface TwGrowthTableRow {
  month: string;
  post_count: string;
...
export const TW_GROWTH_ANALYST_SYSTEM_PROMPT = `
...
Identify the most significant performance outcome from the monthly data. What moved the most across the period — impressions, followers growth, or engagement? Brief — exists only to ground the analysis.

Layer 2 — WHY IT HAPPENED
This is where you earn your value. Cross-reference metrics to detect patterns:
- Do months with high impressions also show high engagement, or do they decouple (high impressions, passive audience)?
- Does followers growth track impressions — when impressions spike, does follower growth follow?
...
Translate detected patterns into strategic meaning about audience behavior and content health for this Twitter/X account. What does the trend imply about distribution reach, audience conversion, and content engagement quality? Stay within what the data supports.
...
- Flag divergence: high impressions + low engagement signals passive viewership, not active audience
- Flag divergence: high impressions + low followers growth signals reach without conversion
- GAP rows (MoM or YoY change rows) are acceleration indicators — use them to assess momentum direction
- Post count relative to impressions reveals content efficiency — fewer posts with higher impressions = better signal
...
- Impressions     : total times posts were displayed to users

Growth Metrics:
- Followers Growth: net new followers gained per month
...
export function buildTwGrowthPrompt(
  clientName: string,
  period: string,
  tableRows: TwGrowthTableRow[],
): string {
  const dataLines = tableRows
    .map(
      (r) =>
        `${r.month}: Posts=${r.post_count}, FollowersGrowth=${r.followers_growth}, Impressions=${r.impressions}, Engagement=${r.engagement}`,
    )
    .join('\n');

  return (
    `Twitter/X Growth Analysis\n` +
    ...
    `Flag any months where impressions and engagement decouple, or where impressions do not translate to follower growth. ` +
    `Assess content efficiency by comparing post count against impressions and engagement. ` +
...
export function parseTwGrowthInsight(raw: string): ParsedInsight {
  const lines = raw
    .split('\n')
    .map((l) => l.trim())
    .filter(Boolean);
