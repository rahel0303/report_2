...
// Slides 13 & 14 from instagram_visualization
// Content Pillar: Average Reach + Average ER Reach (current vs previous month)
// aon=true  → Slide 13 (AON / brand-owned content)
// aon=false → Slide 14 (Non-AON / client content)
...
export const IgCpReachSlide: React.FC<Props> = ({
  config,
  aon,
  isThumbnail = false,
  currentPage,
  ...
  useEffect(() => {
    ...
    setError(null);
    fetch(
      `/api/innercircle/ig-cp-reach?brand=${encodeURIComponent(config.clientName)}&period=${encodeURIComponent(config.period)}&aon=${aon}`,
    )
      .then((r) => r.json())
      .then((d) => {
  ...
  const slideTitle = `Content Pillar Reach — ${aonLabel}`;
  ...
  const reachData = data ? buildChartData(data.reachCurr, data.reachPrev) : [];
  const erData = data ? buildChartData(data.erCurr, data.erPrev) : [];

  const allPillars = data
    ? Array.from(
        new Set([
          ...data.reachCurr.map((r) => r.content_pillar),
          ...data.reachPrev.map((r) => r.content_pillar),
        ]),
      ).sort()
    : [];
  ...
  return (
    ...
    >
      ...
      <div className="px-5 pt-4 shrink-0">
        ...
        >
          ...
          <div className="pl-3">
            ...
            >
              <Activity size={9} style={{ color: colorPrimary }} />
              {config.period} · Avg Reach &amp; Avg ER Reach per Pillar
            </p>
          </div>
                ...
                <div className="flex h-[calc(100%-24px)] px-1 gap-1">
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <div className="flex-1 min-h-0">
                      <PillarBarChart
                        data={reachData.map((d) => ({ name: d.name, value: d.prev }))}
                        color={BAR_COLOR}
                        isDark={isDark}
                      />
                    </div>
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <div className="flex-1 min-h-0">
                      <PillarBarChart
                        data={reachData.map((d) => ({ name: d.name, value: d.curr }))}
                        color={colorPrimary}
                        isDark={isDark}
                      />
                    </div>
              ...
              >
                ...
                <div className="flex h-[calc(100%-24px)] px-1 gap-1">
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <div className="flex-1 min-h-0">
                      <PillarBarChart
                        data={erData.map((d) => ({ name: d.name, value: d.prev }))}
                        color={BAR_COLOR}
                        isDark={isDark}
                        isER
                      />
                  ...
                  <div className="flex-1 flex flex-col min-h-0">
                    ...
                    <div className="flex-1 min-h-0">
                      <PillarBarChart
                        data={erData.map((d) => ({ name: d.name, value: d.curr }))}
                        color={colorPrimary}
                        isDark={isDark}
                        isER
                      />
      ...
      {data && (
        <div
          style={{ display: 'none' }}
          data-ig-cp-reach={JSON.stringify({
            aon,
            currMonth: data.currMonth,
            prevMonth: data.prevMonth,
            reachCurr: data.reachCurr,
            reachPrev: data.reachPrev,
            erCurr: data.erCurr,
            erPrev: data.erPrev,
            deltaReach: data.deltaReach,
            deltaEr: data.deltaEr,
          })}
        />
      )}
