...
const TW_ORANGE = '#E67E22'; // New Follows — left axis
const TW_BLUE = '#3B82F6'; // Impressions  — right axis
const TW_BAR = '#2980B9'; // Engagement bar

const TABLE_HEADERS = ['Month', 'Post Count', 'Followers Growth', 'Impressions', 'Engagement'];
...
export const TwGrowthSlide: React.FC<Props> = ({
  config,
  isThumbnail = false,
  currentPage,
  totalPages,
  ...
  useEffect(() => {
    ...
    setError(null);
    fetch(
      `/api/innercircle/tw-growth?brand=${encodeURIComponent(config.clientName)}&period=${encodeURIComponent(config.period)}`,
    )
      .then((r) => r.json())
      .then((d) => {
  ...
  if (isThumbnail) {
    return (
      ...
      >
        ...
        >
          {config.clientName || '—'} — Twitter Growth
        </p>
      </div>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="px-5 pt-4 shrink-0">
        ...
        >
          ...
          <div className="pl-3">
            ...
            >
              <BarChart2 size={9} style={{ color: colorPrimary }} /> {config.period} · New Follows ·
      ...
      <div
        style={{ display: 'none' }}
        data-ic-tw-chart={JSON.stringify(mapped)}
        data-ic-tw-table={JSON.stringify(tableRows)}
        data-ic-tw-insight={insight}
      />
